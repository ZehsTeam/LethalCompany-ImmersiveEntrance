...
namespace com.github.zehsteam.ImmersiveEntrance.Helpers.IL;

/**
 * Thank you Zaggy1024
 * All of the code in this file is from https://github.com/Zaggy1024/LC_OpenBodyCams
...
internal interface ILMatcher
{
    ...
    {
        if (field == null)
            Logger.LogWarning($"Field passed to ILMatcher.Ldfld() was null at {sourceFilePath}:{sourceLineNumber} ({callerName})");
    ...
    {
        if (field == null)
            Logger.LogWarning($"Field passed to ILMatcher.Ldsfld() was null at {sourceFilePath}:{sourceLineNumber} ({callerName})");
    ...
    {
        if (field == null)
            Logger.LogWarning($"Field passed to ILMatcher.Stfld() was null at {sourceFilePath}:{sourceLineNumber} ({callerName})");
    ...
    {
        if (field == null)
            Logger.LogWarning($"Field passed to ILMatcher.Stsfld() was null at {sourceFilePath}:{sourceLineNumber} ({callerName})");
    ...
    {
        if (method == null)
            Logger.LogWarning($"Method passed to ILMatcher.Callvirt() was null at {sourceFilePath}:{sourceLineNumber} ({callerName})");
    ...
    {
        if (method == null)
            Logger.LogWarning($"Method passed to ILMatcher.Call() was null at {sourceFilePath}:{sourceLineNumber} ({callerName})");
    ...
    {
        if (ctor == null)
            Logger.LogWarning($"Constructor passed to ILMatcher.Newobj() was null at {sourceFilePath}:{sourceLineNumber} ({callerName})");
...
internal class DebuggingMatcher(ILMatcher matcher) : ILMatcher
{
    ...
    public bool Matches(CodeInstruction instruction)
    {
        var isMatch = matcher.Matches(instruction);
        if (isMatch)
            Logger.LogInfo($"{matcher} matched {instruction}");
        return isMatch;
    }
}
